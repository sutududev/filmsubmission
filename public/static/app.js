...
async function loadCast(id){ const rows=await api(`/api/titles/${id}/cast`); const wrap=document.getElementById('cast'); wrap.innerHTML=''; rows.forEach(r=>{ const row=h('div',{class:'flex items-center justify-between p-2 border-b'}, `${r.name} · ${r.role||''}`, h('div',{}, h('button',{class:'text-red-600', onclick: async ()=>{ await api(`/api/cast/${r.id}`,{method:'DELETE'}); loadCast(id) }},'delete'))); wrap.appendChild(row) }) }
...
window.APP={ loadTitles, loadTitlesFiltered, createTitle, loadUsage, loadArtworks, loadCaptions, loadDocuments, uploadMultipart, loadProfile, saveProfile, loadAvails, createAvail, wizardOpen, wizardClose, loadTasks, loadCast, addCast, loadCrew, addCrew, loadFestivals, addFestival }